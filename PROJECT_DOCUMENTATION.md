...
| :--- | :--- | :--- | :---: | :--- |
| **Requirement 1** | Read PDFs/documents, retrieve relevant content, answer queries with citations. | **Document Research Agent (RAG)**: Multi-format document parser (PDF, DOCX, TXT), recursive character chunker, 128-d dense semantic vector embeddings (L2-normalized cosine similarity) + Okapi BM25 hybrid ranking, and grounded LLM answer generator with fallback. | **`PARTIAL`** *(Pipeline fully verified; live LLM completion blocked by upstream OmniRoute proxy outages)* | `backend/agents/document_agent.py`<br>`backend/rag/parsers.py`<br>`backend/rag/chunker.py`<br>`backend/rag/vector_store.py` |
